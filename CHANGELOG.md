...
- `POST /flussonic-servers/:serverId/streams/sync` — pulls the server's real, cursor-paginated `GET streams` endpoint (follows `next` until exhausted) and reconciles it with `flussonic_streams`: a known stream (matched by name) gets its new `live_stats_json` column refreshed (live bitrate, client count, media tracks, per-input stats, etc. — stored verbatim, not modeled field-by-field); a stream found upstream with no local row is imported using its `config_on_disk` as the seed `config_json`. Existing rows' `config_json` is left untouched by sync — it stays "what we'll PUT back on the next edit", separate from the live snapshot. Returns `{ total, created, updated }`.
