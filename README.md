...
`/api/v1/flussonic-servers/sync-all` — `POST`, no body. Runs all three real-Flussonic sync calls (stats, streams, sessions — see below) for every non-deleted server, one server at a time; any sync type failing for any server never blocks the others. Returns `{ total, results: [{ serverId, name, stats: {ok, error?}, streams: {ok, error?, created?, updated?}, sessions: {ok, error?, created?, updated?} }] }`.
