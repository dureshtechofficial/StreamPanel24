...
- `/dashboard/servers/[id]/streams` — full CRUD for one server's streams (reached via the broadcast icon on its row). The form covers the real Flussonic stream shape: name (application/key, editable — changing it renames the stream), title/comment, static/disabled toggles, a dynamic list of inputs, all 18 protocol flags as switches, and optional `on_play`/`on_publish` auth-hook sections that expand only once enabled. A "Sync" button pulls the server's actual live stream list; the eye icon on a row opens a read-only details panel showing status, bitrate, uptime, and media tracks from that synced data.
- `/dashboard/servers/[id]/sessions` — reached via the "Sessions" button on the streams page. Paginated log of stream sessions pulled from the server's real `GET sessions` endpoint (its own "Sync" button), showing stream name, type, protocol, IP, geolocation/ISP, and start/update times.
- `/dashboard/settings` — admin-only (sidebar "Settings" link, enabled for admins instead of the "Soon" placeholder). One card per sync type (server stats, streams, sessions): an enable toggle, a cron expression field with quick presets (every 5/15/30 min, hourly, every 6h, daily), and the last run's relative time + result summary. Saving takes effect immediately — no restart needed.
