...
function toFormState(reseller: Reseller | null): FormState {
  if (!reseller) return EMPTY_FORM;
  return {
    name: reseller.name,
...
function toPayload(form: FormState): ResellerInput {
  return {
    name: form.name.trim(),
...
export function ResellerFormPanel({
  open,
  reseller,
  onClose,
  onSubmit,
  ...
  onClose: () => void;
  onSubmit: (payload: ResellerInput) => Promise<void>;
}) {
  const [form, setForm] = useState<FormState>(() => toFormState(reseller));
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!reseller && form.password.length < 8) clientErrors.password.push('Password must be at least 8 characters long');
  ...
  return (
    <Sheet open={open} onOpenChange={(next) => { if (!next) onClose(); }}>
      <SheetContent size="md">
        <SheetHeader>
          <SheetTitle>{reseller ? 'Edit reseller' : 'Add reseller'}</SheetTitle>
          <SheetDescription>
            {reseller ? 'Update this reseller account.' : 'Create a new reseller account.'}
          </SheetDescription>
        </SheetHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-1 flex-col overflow-hidden">
          <SheetBody>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={reseller ? 'Leave blank to keep unchanged' : undefined}
                />
            ...
            <div>
              ...
              <select
                value={form.status}
                onChange={(e) => setField('status', e.target.value as ResellerStatus)}
                className={inputClass}
              >
          ...
          <SheetFooter>
            ...
            <Button type="submit" size="sm" loading={isSubmitting}>
              {isSubmitting ? 'Saving…' : 'Save reseller'}
            </Button>
          </SheetFooter>
        </form>
      </SheetContent>
